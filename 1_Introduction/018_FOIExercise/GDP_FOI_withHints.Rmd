...
```{r WDIdata}
WDIdf <- WDI(indicator=c("NY.GDP.PCAP.PP.KD","IT.NET.USER.ZS","SP.POP.TOTL"), country ="all", start = 2014, end = 2014, extra = TRUE)
```
...
```{r selectCountries}
newdf_5M = newdf[newdf$IT.NET.USER.ZS/100*newdf$SP.POP.TOTL > 5000000,]
```
...
```{r trendsData}
geoMapDF <- read.csv("geoMap.csv", header = TRUE, sep = ",", quote = "\"", stringsAsFactors = FALSE, skip=2)
geoMapDF_clean = geoMapDF[complete.cases(geoMapDF) & geoMapDF$X2013...2014.!="",]
```
...
```{r FOI}
geoMapDF_clean$FOI = as.numeric(sub("%", "",geoMapDF_clean$X2015...2014.)) / as.numeric(sub("%", "",geoMapDF_clean$X2013...2014.))
```
...
"merge" or "inner_join")
```{r MergeData}
allDF <- merge(geoMapDF_clean, newdf_5M, by.x="Country", by.y="country")
```
...
```{r correlation}
GDP_FOI_cor = cor(allDF$FOI, allDF$NY.GDP.PCAP.PP.KD, method="pearson")
```
...
plot(shufdata$FOI, allDF$NY.GDP.PCAP.PP.KD)
shufdata_cor = cor(shufdata$FOI, allDF$NY.GDP.PCAP.PP.KD, method="pearson")
```
